// atelier/adapters/codex/adapter.ts — ADAPTER #2, built now rather than "ported later".
//
// Codex consumes the same open Agent Skills format (SKILL.md + optional references/ scripts/) and
// supports plugins and lifecycle hooks including PreToolUse. So this adapter reuses the IDENTICAL
...
export class CodexAdapter implements HostAdapter {
  detect(): HostCapabilities {
    return { hostId: 'codex', skills: true, plugins: true, blockingHooks: true, persistentPluginData: true, invocationStyle: '$' };
  }
  private dir(projectDir: string, skillId: string): string { return join(projectDir, '.codex', 'skills', skillId); }

  /**
   * WHAT Codex ITSELF DELIVERS, when a user invokes the skill natively instead of through Atelier.
   *
  ...
  /** Codex selects skills with a $ mention. Different punctuation, same skill. */
  ...
  installProtocolGuards(policy: ProtocolPolicy): GuardResult {
    ...
    return { installed: false, enforcedBy: 'NOT_INSTALLED', artifact: null,
      detail: `Codex can enforce a blocking pre-tool hook; Atelier does not install one yet, so the `
  ...
  persistentStateLocation(): string { return process.env.CODEX_PLUGIN_DATA ?? join(process.env.HOME ?? '.', '.atelier'); }
}
